...
std::string UI::GetHelloWorld() { return "**** Hello World ****"; }
std::string UI::GetLongText() {
  // return "1234 This is a very long text. Use this to demo a long text! 56789";
  return "Lorem Ipsum is simply dummy text. Lorem Ipsum has been the industry's standard dummy text ever since the 1500s";
}

void UI::drawcircle(int i0, int j0, int radius) {
  int i = radius;
  int j = 0;
  int err = 0;
...
bool UI::DrawLine(const int i0, const int j0, const int i1, const int j1,
...
bool UI::DrawLine(const Point &start, const Point &end,
                        const std::string &str) const {
  return DrawLine(start.GetI(), start.GetJ(), end.GetI(), end.GetJ(), str);
}

bool UI::DrawSevenSegment(const int i0, const int j0, const int width,
...
void UI::PrintStringOnSevenSegment(int i, int j, int width, int height,
